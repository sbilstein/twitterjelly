...
class TweetFetcher:   
    ...
    def fetchRateData(self):
        return json.loads(urllib.urlopen("https://api.twitter.com/1/account/rate_limit_status.json").read().decode("ascii"))
    ...
    def getUserTweetsData(self,user):        
        ...
        twitterquery = urllib.quote(twitterquery)
        print(twitterquery)
        queryurl = "http://search.twitter.com/search.json?lang=en&rpp=100&q=%s"%twitterquery
        print(queryurl)
        data = json.loads(urllib.urlopen(queryurl).read().decode("ascii"))
